...
### Added
- `config/distube.js` now logs ffmpeg's real stderr (filtered to error-looking lines) via DisTube's `ffmpegDebug` event, which nothing previously listened to. An `FFMPEG_EXITED` error only ever reported the exit code, never *why* — this surfaces that reason in the logs going forward.

### Fixed

- **`FFMPEG_EXITED` on every playback attempt, `HTTP error 403 Forbidden` on the googlevideo URL** (found via the new `ffmpegDebug` logging above): googlevideo signs the media URL to the IP yt-dlp resolved it from; on dual-stack (IPv4+IPv6) hosts — common on VPS/Docker — that resolution and ffmpeg's separate fetch of the URL moments later can go out over different IP versions, and the mismatch gets the fetch rejected. Added `--force-ipv4` to the yt-dlp config (the documented fix for this exact symptom) so resolution is consistent with how most hosts actually reach the CDN. Distinct from the bot-check 403 ("Sign in to confirm...", fails at resolution, before ffmpeg even starts) — this one is a bare "Forbidden" from ffmpeg itself.
- **YouTube songs start a few seconds in instead of from the beginning**: the `tv` and `web_safari` player clients (added to the bot-check fallback chain above) serve audio as HLS (`m3u8`) by default, which ffmpeg doesn't always demux from the true start. Added `--format-sort proto:https` to the yt-dlp config so it prefers the clean progressive `https` format `android` already gave us (confirmed via the real yt-dlp binary: same format, `18`/`mp4a.40.2`, as before the `tv`/`web_safari` clients were added) whenever one's available, without dropping `tv`/`web_safari` from the client list — they're still needed for the bot-check itself.
