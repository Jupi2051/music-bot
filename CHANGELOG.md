...
### Added
- Cookie upload web UI (`web/cookieServer.js`, opt-in via `COOKIE_SERVER_PORT`): a token-gated page with a step-by-step tutorial (Get cookies.txt LOCALLY / Cookie-Editor) and a form to submit a Netscape-format `cookies.txt` without shell access. Saving it hot-reloads yt-dlp's config immediately (no restart) via the shared `scripts/write-ytdlp-config.js`, which `entrypoint.sh` also now delegates to instead of hardcoding the config in shell. `docker-compose.yml`'s `cookies.txt` mount is writable (was `:ro`) and maps the new port; see the README's "YouTube cookies" section for the ownership gotcha (`touch cookies.txt` before the first `docker compose up`).
