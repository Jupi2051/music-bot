...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (seconds === null || !Number.isFinite(duration)) {
      return interaction.reply({ content: '❌ Use a valid time in mm:ss format (for example, 3:20).', ephemeral: true });
    }
    if (seconds > duration) {
      return interaction.reply({ content: `❌ The time cannot exceed the song duration (${queue.songs[0].formattedDuration}).`, ephemeral: true });
    }
    ...
    try {
      ...
      await interaction.reply('❌ Could not change the playback position.').catch(() => {});
    }
  },
};
