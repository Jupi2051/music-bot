...
| F4.1 Estrategia de testing | ✅ | `node:test` (sin dependencias): 62 tests en `test/` (helpers 7, comandos 31+2 cleanQuery, update-commands 5 con orden hash-vs-deploy, distube-events 12, entrypoint 4). `npm test` = `node --test "test/*.test.js"` (Node 22.14 no acepta directorio como arg). Faltan: red real de Discord (integración opt-in) e interactionCreate/ready |
| F4.2 Tests de integración opt-in | ✅ | `test/integration/ytdlp.test.js` con red real (YouTube + binario yt-dlp local), activados con `npm run test:integration` (env `TEST_INTEGRATION=1`; con `npm test` quedan skipped). Cubren los 2 fallos de producción de ago-2026: video individual y radio RD acotada. Verificados en vivo 2026-08-02: 2/2 OK (video ~39s, radio ~43s) |
| F4.3 Bugfix producción: query pegado + radios RD | ✅ | **Bug A (NO_RESULT)**: `/play` fallaba con `DisTubeError [NO_RESULT]` al pegar el texto del mensaje del bot (`🔍 Buscando: \`URL\`` con backticks) como query → isURL=false → rama #searchSong → sin resultados. Fix: `cleanQuery()` en `utils/helpers.js` extrae la primera URL `https?://...` o quita backticks envolventes (commit `efbfb7e`). **Bug B (radio colgada)**: URLs `list=RD...&start_radio=1` son listas virtuales ILIMITADAS; el plugin usa `--dump-single-json` y nunca terminaba → bot colgado en "🔍 Buscando:" sin error. Fix: `--playlist-end 25` en config yt-dlp de `entrypoint.sh` (commit `1f7b9d3`). Verificado en producción por el usuario. Síntoma distinto = causa distinta: colgado sin error = extracción infinita; error NO_RESULT = input no-URL |
| F4.4 clientReady + limpieza deprecation | ✅ | `client.on('ready')` → `client.on('clientReady')` en `index.js` y `events/ready.js` (discord.js v15 lo exige; elimina el DeprecationWarning del log) |
